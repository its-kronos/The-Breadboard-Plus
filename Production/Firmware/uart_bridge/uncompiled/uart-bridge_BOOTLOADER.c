...
int main() 
{
    ...
    const uint WROOM_ENABLE = 3;
    const uint WROOM_IO2 = 0;
    const uint WROOM_IO8 = 4;
    const uint WROOM_IO9=5;
    
    gpio_init(WROOM_ENABLE);
    gpio_init(WROOM_IO2);
    gpio_init(WROOM_IO8);
    gpio_init(WROOM_IO9);
    
    gpio_set_dir(WROOM_ENABLE, GPIO_OUT);
    gpio_set_dir(WROOM_IO2, GPIO_OUT);
    gpio_set_dir(WROOM_IO8, GPIO_OUT);
    gpio_set_dir(WROOM_IO9, GPIO_OUT);
    
    gpio_put(WROOM_ENABLE, false);
    
    gpio_put(WROOM_IO2, true);
    gpio_put(WROOM_IO8, true);
    gpio_put(WROOM_IO9, false);
    
    sleep_ms(100);
    gpio_put(WROOM_ENABLE, true);
